...
function requireSocialApiUrl(raw: string | undefined): string {
  if (!raw || !raw.trim()) {
    throw new Error(
      `[SocialAuth] EXTERNAL_API_URL is not set. This environment variable is required and must point to https://<host>.${ALLOWED_API_DOMAIN}`
    );
  }
  ...
  try {
    ...
    if (host !== ALLOWED_API_DOMAIN && !host.endsWith(`.${ALLOWED_API_DOMAIN}`)) {
      throw new Error(
        `[SocialAuth] EXTERNAL_API_URL host "${host}" is not allowed. Must be ${ALLOWED_API_DOMAIN} or a subdomain.`
      );
    }
  } catch (e: any) {
    if (e?.message?.startsWith("[SocialAuth]")) throw e;
    throw new Error(`[SocialAuth] EXTERNAL_API_URL is not a valid URL: ${raw}`);
  }
...
let _cachedExternalApi: string | null = null;
function getExternalApis(): string[] {
  if (!_cachedExternalApi) {
    try {
      _cachedExternalApi = requireSocialApiUrl(
...
async function fetchExternalWithFallback(path: string, options: RequestInit): Promise<globalThis.Response> {
  ...
  for (const base of getExternalApis()) {
